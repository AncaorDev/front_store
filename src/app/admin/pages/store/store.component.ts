...
@Component({
    selector: "app-store",
    templateUrl: "./store.component.pug",
...
export class StoreComponent implements OnInit {
    ...
    async loadDataList() {
        try {
            ...
            const data = await this.storeSrv.getAll().toPromise();
    ...
    create() {
        return this.router.navigate(["/admin/stores/create"]);
    }

    edit(element) {
        return this.router.navigate(["/admin/stores/edit/"+element.id_store]);
    }

    async remove(element) {
        this.loading.changeStatus(true);
        try {
            await this.storeSrv.remove(element.id_store).toPromise();
            this.loadDataList();
            this.loading.changeStatus(false);
        } catch (err) {
            this.showError(err)
